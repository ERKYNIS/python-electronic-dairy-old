    ...
    try:
        ...
        try:
            ...
            await update.message.reply_text(
                "Произошла ошибка!\n"
                f"Повторите попытку через несколько секунд.")
...
async def link(update, context):
    if len(context.args) > 0:
        ...
        if "account" in context.user_data:
            ...
            return await update.message.reply_text(f"К вашему аккаунту уже привязан аккаунт {account[4]} {account[5]}!")
...
def startbot():
    application = Application.builder().token("ТОКЕН БОТА TELEGRAM").build()
